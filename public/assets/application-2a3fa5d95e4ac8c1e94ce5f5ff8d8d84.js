...
app.modals.deleteResearcher = '<div class="modal-actual"><h2 class="modal-title"><%- m.researcher.first_name %> <%- m.researcher.last_name %></h2><h3 class="modal-subtitle">Are you sure you want to delete this researcher?</h3><h3 class="modal-subtitle">This action will destroy all trials associated with this researcher!</h3><p class="modal-instruction">Enter researcher name and press delete:</p><input type="text" class="delete-confirmation" /><button class="mdl-btn confirm-delete">DELETE</button><button class="mdl-btn cancel-delete">CANCEL</button></div>';
app.researcherDetail = '<div class="r-detail-cntnr"><h1 class="r-detail-name"><%- m.researcher.first_name %> <%- m.researcher.last_name %></h1><h2 class="r-detail-email"><%- m.researcher.email %></h2><ul class="rslts-list"></ul><div class="btn-wrp"><button class="reg-btn r-btn bck-btn"><span class="icon">a</span> back</button><button class="reg-btn r-btn del-btn">delete</button><button class="reg-btn r-btn r-edit">edit</button></div>';
app.researcherListing = '<ul><% m.drs.forEach(function(dr) { %><li data="<%- dr.id %>"class="researcher-listing"><span class="r-listing-info r-name"><%- dr.first_name %> <%- dr.last_name %></span><span class="r-listing-info"><%- dr.email %></span><span class="r-listing-info">Active Trials:</span></li><% }) %></ul>';
app.trialDetail = {};

app.trialDetail.editable = '<div class="t-detail-container"><button class="reg-btn bck-btn"><span class="icon">a</span> back</button><h1 class="t-detail-header"><%- m.trial.name %></h1><h2 class="t-detail-subhead"><%- m.trial.conditions[0].name %></h2><h2 class="t-detail-org"><%- m.trial.organization[0].name %>, <%- m.trial.location %></h2><div class="sub-dtl-wrap"><h3 class="t-detail-subtitle">principal investigator: <%- m.trial.principal %></h3><span class="t-detail-date">start date: <%- m.trial.start_on %></span><span class="t-detail-date">, est. completion: <%- m.trial.estimated_completed_on %></span><p class="t-detail-description"><%- m.trial.description %></p><p class="t-detail-contact">primary contact: <%- m.trial.primary_contact_email %></p></div></div><div class="btn-wrp"><button class="reg-btn td-btn del-btn">delete study</button><button class="reg-btn td-btn edit-btn">edit study</button><button class="reg-btn td-btn cls-btn">close study</button></div></div>';

app.trialDetail.readable = '<div class="t-detail-container"><button class="reg-btn bck-btn"><span class="icon">a</span> back</button><h1 class="t-detail-header"><%- m.trial.name %></h1><h2 class="t-detail-subhead"><%- m.trial.conditions[0].name %></h2><h2 class="t-detail-org"><%- m.trial.organization[0].name %>, <%- m.trial.location %></h2><div class="sub-dtl-wrap"><h3 class="t-detail-subtitle">principal investigator: <%- m.trial.principal %></h3><span class="t-detail-date">start date: <%- m.trial.start_on %></span><span class="t-detail-date">, est. completion: <%- m.trial.estimated_completed_on %></span><p class="t-detail-description"><%- m.trial.description %></p><p class="t-detail-contact">primary contact: <%- m.trial.primary_contact_email %></p></div></div>';
app.trialListing = {};
app.trialListing.search = '<li class="rslts-hdr rslt-listing"><span class="rslt-info rslt-hdr-info">trial name</span><span class="rslt-info rslt-hdr-info">conditions</span><span class="rslt-info rslt-hdr-info">organization</span><span class="rslt-info rslt-hdr-info">location</span><span class="rslt-info rslt-hdr-info">start date</span></li><% m.results.forEach(function(result, i) { %><li data="<%- result.id %>"class="rslt-listing"><span class="rslt-info rslt-name" data="<%- i %>"><%- result.name %></span><span class="rslt-info"><%- result.conditions[0].name %></span><span class="rslt-info"><%- result.organization[0].name %></span><span class="rslt-info"><%- result.location %></span><span class="rslt-info"><%- result.start_on %></span></li><% }) %>';
